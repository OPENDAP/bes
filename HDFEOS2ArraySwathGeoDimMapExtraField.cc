...
HDFEOS2ArraySwathGeoDimMapExtraField::read ()
{
	...
	int32 swathnamesize;
	numswath =
		inqfunc (const_cast < char *>(filename.c_str ()), NULL,
				 &swathnamesize);
	...
	if (numswath == -1) {
		...
		eherr << "File " << filename.
			c_str () << " cannot obtain the swath list.";
		throw InternalErr (__FILE__, __LINE__, eherr.str ());
	}
	...
	numswath =
		inqfunc (const_cast < char *>(filename.c_str ()), swathname,
				 &swathnamesize);
	if (numswath == -1) {
		delete[]swathname;
		HDFCFUtil::ClearMem (offset32, count32, step32, offset, count,
...
int
HDFEOS2ArraySwathGeoDimMapExtraField::format_constraint (int *offset,
														 int *step,
														 int *count)
{
	long nels = 1;
