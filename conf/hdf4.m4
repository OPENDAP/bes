...
dnl args :             action-if-yes, action-if-no, path to check
dnl
dnl Modified to take a third parameter that gives a path to check
dnl as the installed location of hdf4, etc. 
dnl The macro will subst the following symbols: HDF4_LDFLAGS, HDF4_CFLAGS,
dnl HDF4_LIBS, HDF4_PATH. jhrg 11/30/14
AC_DEFUN([AC_CHECK_HDF4],
[
  dnl NB: If --with-hdf4 is NOT used, this will be the value of HDF4_PATH
  BES_DEPS_PREFIX=$3    

  AC_ARG_WITH([hdf4],
            [AS_HELP_STRING([--with-hdf4=ARG],[hdf4 directory])],
            [HDF4_PATH=$withval], 
            [HDF4_PATH="$BES_DEPS_PREFIX"])
  ...
  AS_IF([test -n "$H4_CC" -a -x $H4_CC],
      [AC_SUBST(H4_CC)
      AC_MSG_NOTICE(H4_CC: $H4_CC)],
      [H4_CC=""
      AC_SUBST(H4_CC)]
  )

  AC_MSG_NOTICE(--------------- Searching for HDF4 Libraries ------------------------)
  ...
  AC_MSG_NOTICE(--------------- Searching for HDF4 headers ------------------------)
