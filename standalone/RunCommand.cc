 ...
 * @see    BESError
 */
void RunCommand::setOutput(ostream * strm, bool created)
{
	if (_strmCreated && _strm) {
		_strm->flush();
		delete _strm;
 ...
 * @see    BESError
 */
void RunCommand::executeClientCommand(const string &cmd)
{
 ...
 * @see    BESError
 */
void RunCommand::executeCommand(const string & cmd, int repeat)
{
	...
	if (cmd.compare(0, client.size(), client) == 0) {
		...
		for (int i = 0; i < repeat; i++) {
			...
			BESDEBUG( "standalone", "RunCommand::executeCommand sending: " << cmd << endl );
			...
			*_strm << flush;
            ...
            if (status == 0) {
                BESDEBUG("standalone", "RunCommand::executeCommand - executed successfully" << endl);
            }
            else {
                // an error has occurred.
                BESDEBUG("standalone", "RunCommand::executeCommand - error occurred" << endl);
                switch (status) {
 ...
 * @see    BESError
 */
void RunCommand::executeCommands(const string &cmd_list, int repeat)
{
	_isInteractive = true;
	if (repeat < 1) repeat = 1;
 ...
 * @see    BESError
 */
void RunCommand::executeCommands(ifstream & istrm, int repeat)
{
	_isInteractive = false;
	if (repeat < 1) repeat = 1;
	for (int i = 0; i < repeat; i++) {
 ...
 */
size_t RunCommand::readLine(string & msg)
{
	size_t len = 0;
	char *buf = nullptr;
