/* makesds2.c: Creates testfile (testsds1.hdf) for HDF-EOS server
 * Contains SDS's of every data type
 * Values written include 0, 1, -1, max, and min for each datatype
 * Same as makesds1, but uses SD_UNLIMITED for dimension
...
main() {
  ...
  const int32 maxsize[1] = {SD_UNLIMITED};
  ...
  for(int i=0; i<numsds; i++) {
    ...
    sds_id = SDcreate(sd_id, sdsname, tests[i].numtype, 1, maxsize);
    if(sds_id == FAIL)
      return 1;
