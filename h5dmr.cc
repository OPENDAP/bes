...
// So we use this search. In the future, we may just use the breadth_first search for all cases.?? 
//bool breadth_first(hid_t pid, char *gname, DMR & dmr, D4Group* par_grp, const char *fname,bool use_dimscale)
bool breadth_first(hid_t pid, char *gname, D4Group* par_grp, const char *fname,bool use_dimscale,vector<link_info_t> & hdf5_hls )
{
    BESDEBUG("h5",
    ...
    for (hsize_t i = 0; i < nelems; i++) {
        ...
        if(H5O_TYPE_DATASET == obj_type) {
            ...
            get_dataset_dmr(pid, full_path_name, &dt_inst,use_dimscale,is_pure_dim,hdf5_hls);
        ...
        if(obj_type == H5O_TYPE_GROUP) {  
            ...
            if (oid == "") {
                try {
                    ...
                    // Continue searching the objects under this group
                    breadth_first(cgroup, &t_fpn[0], tem_d4_cgroup,fname,use_dimscale,hdf5_hls);
                }
                catch(...) {
                    H5Gclose(cgroup);
                    throw;
