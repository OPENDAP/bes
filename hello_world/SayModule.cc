...
{
    BESDEBUG( "say", "Initializing Say Handler:" << endl )

    BESDEBUG( "say", "    adding " << modname << " request handler" << endl )
    BESRequestHandlerList::TheList()->add_handler( modname, new SayRequestHandler( modname ) ) ;

    BESDEBUG( "say", "    adding " << say_RESPONSE << " response handler" << endl )
    BESResponseHandlerList::TheList()->add_handler( say_RESPONSE, SayResponseHandler::SayResponseBuilder ) ;

    BESDEBUG( "say", "    adding " << say_RESPONSE << " command" << endl )
...
SayModule::terminate( const string &modname )
{
    ...
    BESDEBUG( "say", "    removing " << say_RESPONSE << " response handler" << endl )
