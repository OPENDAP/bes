...
string BESUtil::rfc822_date(const time_t t) {
	...
	snprintf(d, 255, "%s, %02d %s %4d %02d:%02d:%02d GMT", days[stm->tm_wday],
			stm->tm_mday, months[stm->tm_mon], 1900 + stm->tm_year,
...
string BESUtil::www2id(const string &in, const string &escape,
		const string &except) {
 ...
 * path does not exist.
 **/
void BESUtil::check_path(const string &path, const string &root,
...
string BESUtil::implode(const list<string> &values, char delim) {
	...
	for (; i != e; i++) {
		...
		if (d != string::npos && (*i)[0] != '"') {
			string err =
					(string) "BESUtil::implode - delimiter exists in value "
							+ (*i);
			throw BESInternalError(err, __FILE__, __LINE__);
		}
		//d = string::npos ;
