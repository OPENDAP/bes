...
ServerAdministrator::ServerAdministrator() {
    ...
    if (d_organization.empty()) {
        BESDEBUG(MODULE, __func__ << "() -  The configuration entry for the " << SERVER_ADMINISTRATOR_KEY << "["
                                  << ORGANIZATION_KEY << "] was missing." << endl);
        bad_flag = true;
    }
    ...
    if (d_region.empty()) {
        BESDEBUG(MODULE, __func__ << "() -  The configuration entry for the " << SERVER_ADMINISTRATOR_KEY << "["
                                  << REGION_KEY << "] was missing." << endl);
        d_region = get(STATE_KEY);

        if (d_region.empty()) {
            BESDEBUG(MODULE, __func__ << "() -  The configuration entry for the " << SERVER_ADMINISTRATOR_KEY << "["
                                      << STATE_KEY << "] was missing." << endl);
            bad_flag = true;
        }
    }
    ...
    if (d_postal_code.empty()) {
        BESDEBUG(MODULE, __func__ << "() -  The configuration entry for the " << SERVER_ADMINISTRATOR_KEY << "["
                                  << POSTAL_CODE_KEY << "] was missing." << endl);
        bad_flag = true;
    }
    ...
    if (d_telephone.empty()) {
        BESDEBUG(MODULE, __func__ << "() -  The configuration entry for the " << SERVER_ADMINISTRATOR_KEY << "["
                                  << TELEPHONE_KEY << "] was missing." << endl);
        bad_flag = true;
    }
    ...
    if (d_website.empty()) {
        BESDEBUG(MODULE, __func__ << "() -  The configuration entry for the " << SERVER_ADMINISTRATOR_KEY << "["
                                  << WEBSITE_KEY << "] was missing." << endl);
        bad_flag = true;
    }

    // %TODO This is a pretty simple (and brutal) qc in that any missing value prompts all of it to be rejected. Review.
    // Fix?
    if (bad_flag) {
        mk_default();
        BESDEBUG(MODULE, __func__ << "() -  The configuration entry for the " << SERVER_ADMINISTRATOR_KEY
                                  << " was missing crucial information.  jdump(): " << jdump(true) << endl);
    }
}
...
void ServerAdministrator::mk_default() {
    ...
    BESDEBUG(MODULE,
             __func__ << "() - ServerAdministrator values have been set to the defaults:  " << jdump(true) << endl);
}
