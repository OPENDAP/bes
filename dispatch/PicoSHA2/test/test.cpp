...
#define PICOSHA2_CHECK_EQUAL(a, b)                                                                                     \
    {                                                                                                                  \
        if (a == b) {                                                                                                  \
            std::cout << "\033[32m" << __FUNCTION__ << "(LINE:" << __LINE__ << ") is succeeded." << "\033[39m"         \
                      << std::endl;                                                                                    \
        } else {                                                                                                       \
...
template <typename OutIter> void input_hex(std::istream &is, OutIter first, OutIter last) {
    char c;
...
template <typename OutIter> void hex_string_to_bytes(const std::string &hex_str, OutIter first, OutIter last) {
    assert(hex_str.size() >= 2 * std::distance(first, last));
    std::istringstream iss(hex_str);
    input_hex(iss, first, last);
}

template <typename OutContainer> void hex_string_to_bytes(const std::string &hex_str, OutContainer &bytes) {
...
const std::pair<std::string, std::string> sample_message_list[sample_size] = {
    mess_and_hash("", "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"),
...
void test() {
    for (std::size_t i = 0; i < sample_size; ++i) {
        ...
        {
            std::list<unsigned char> hash(32);
            picosha2::hash256(src_vect.data(), src_vect.data() + src_vect.size(), hash.begin(), hash.end());
            PICOSHA2_CHECK_EQUAL_BYTES(ans, hash);
        }
        {
            std::list<unsigned char> hash(32);
