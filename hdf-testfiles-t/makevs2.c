/* makevs2.c: Creates testfile (testvs2.hdf) for HDF-EOS server
...
main() {
  ...
  for(i=0; i<numvd; i++) {
    ...
    case DFNT_CHAR8:
      ...
      buf[0] = 1;	buf[1] = -1;      buf[2] = INT8_MAX;
      buf[3] = INT8_MIN; buf[4] = 0;
      break;
      }
    case DFNT_UCHAR8:
      {uchar8 *buf = (uchar8*)buffer = malloc(sizeof(uchar8)*3);
      buf[0] = 1;	buf[1] = UINT8_MAX;  buf[2] = 0;
      break;
      }
    }
    if((vdata_ref = VHstoredatam(file_id, vdfield, (uint8 *)buffer, 1,
	     tests[i].numtype, vdname, vdclass, tests[i].numelts))==FAIL)
      return 2;
  }
