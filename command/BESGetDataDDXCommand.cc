...
#include "BESGetDataDDXCommand.h"
 ...
 */
BESResponseHandler *BESGetDataDDXCommand::
parse_request(BESTokenizer & tokenizer, BESDataHandlerInterface & dhi)
{
    ...
    if( my_token != "contentStartId" )
    {
	tokenizer.parse_error(my_token +
			  " not expected, expecting \"contentStartId\"");
    }

    starting = tokenizer.get_next_token();

    my_token = tokenizer.get_next_token();
    if( my_token != "mimeBoundary" )
    {
	tokenizer.parse_error(my_token +
			      " not expected, expecting \"mimeBoundary\"" );
    }

    bounding = tokenizer.get_next_token() ;
    ...
    dhi.data[DATADDX_STARTID] = starting;
    dhi.data[DATADDX_BOUNDARY] = bounding;
