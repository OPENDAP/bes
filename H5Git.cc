...
/// \return NotGrid, NewH4H5Grid  (values of H5GridFlag_t)
/// Note: The dimensional scale data following the old version of H4toh5 utility 
/// will not be supported since no production files are generated by those files
///////////////////////////////////////////////////////////////////////////////
H5GridFlag_t maptogrid(hid_t dataset, int num_dim)
{

    // Check if we have HDF5 dimension scale data following H4toH5 mapping doc.
    // http://www.hdfgroup.org/HDF5/doc/ADGuide/H4toH5Mapping.pdf
    if (map_to_grid(dataset, num_dim))
        return NewH4H5Grid;
    else
        return NotGrid;
}
...
/// \return a number
///////////////////////////////////////////////////////////////////////////////
int map_to_grid(hid_t dataset, int num_dim)
{
    ...
    //1.  Check whether we have attribute "HDF5_DIMENSIONLIST" , 
    //if not, map the dataset to DAP array.
	...
	if (attr_id < 0){
	   throw InternalErr(__FILE__, __LINE__, "cannot open the attribute specified");
        ...
        if(!strncmp(dimscale, HDF5_DIMENSIONLIST,
    ...
    // 1.2 Check whether we have attribute "DIMENSION_NAMELIST", if no, map the HDF5 dataset
    // to DAP array.
    for (i = 0; i < (unsigned int) num_attrs; i++) {
        attr_id = H5Aopen_idx(dataset, i);
        if (attr_id < 0){
        ...
        if(!strncmp(dimscale, HDF5_DIMENSIONNAMELIST,
	    ...
	    // number of element for HDF5 dimensional object reference array
            // is the number of dimension of the corresponding HDF5 array. 
            ssiz = H5Sget_simple_extent_npoints(space);
             if (ssiz == 0){
               throw InternalErr(__FILE__, __LINE__, "Invalid number of elements in the dataspace");
            }
    ...
    /// We want to check if the number of dimension for Dimension_name_list and Dimension_list is the same.
    /// If not, we cannot map this HDF5 dataset to DAP Grid. 
...
bool has_matching_grid_dimscale(hid_t dataset, int ndims, int *sizes)
{
    ...
    // is not mature. We may need to revise that if HDF5 dimension scale is revised. KY 2009/07/09
