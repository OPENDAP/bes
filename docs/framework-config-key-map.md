...
- Excluded on purpose: test-only `bes.conf.in` files, module-specific handler configs under `modules/`, retired code except where a key is only referenced there
...
- A few keys in `dispatch/bes/bes.conf.in` are consumed by code outside the four framework directories, mainly under `server/` and `ppt/`. I kept those because they are part of the installed framework config.
...
| `BES.LogName` | raw string `"BES.LogName"` | `dispatch/BESLog.cc:98` | Primary log file path. |
...
| `BES.Catalog.catalog.RootDirectory` | dynamic key pattern `"BES.Catalog." + n + ".RootDirectory"` | `dispatch/BESCatalogUtils.cc:79`; also `http/RemoteResource.cc:53`, `dap/BESStoredDapResultCache.cc:86` | `catalog` is the catalog name used in the template. |
...
| `AllowedHosts` | `ALLOWED_HOSTS_BES_KEY` in `http/AllowedHosts.h:39` | `http/AllowedHosts.cc:54`, `http/CurlUtils.cc:1151` | Controls remote URL allow-listing. |
| `BES.Data.RootDirectory` | raw string `"BES.Data.RootDirectory"` and `BES_DATA_ROOT` macro in `dap/BESStoredDapResultCache.cc:85` | `dispatch/BESContainerStorageVolatile.cc:74`, `dap/BESStoredDapResultCache.cc` | Standalone-BES data root. |
...
| `BES.Include` | `BES_INCLUDE_KEY` in `dispatch/TheBESKeys.cc:52` and `dispatch/kvp_utils.cc:46` | `dispatch/TheBESKeys.cc`, `dispatch/kvp_utils.cc` | Include/ingest other config files. |
...
| `DAP.GlobalMetadataStore.path` | `PATH_KEY` in `dap/GlobalMetadataStore.cc:98` | `dap/GlobalMetadataStore.cc:267` | Comment says unset/null disables MDS. |
| `DAP.GlobalMetadataStore.prefix` | `PREFIX_KEY` in `dap/GlobalMetadataStore.cc:99` | `dap/GlobalMetadataStore.cc:251` | |
| `DAP.GlobalMetadataStore.size` | `SIZE_KEY` in `dap/GlobalMetadataStore.cc:100` | `dap/GlobalMetadataStore.cc:236` | |
| `DAP.GlobalMetadataStore.ledger` | `LEDGER_KEY` in `dap/GlobalMetadataStore.cc:101` | `dap/GlobalMetadataStore.cc:370` | |
...
| `Http.cache.effective.urls` | `HTTP_CACHE_EFFECTIVE_URLS_KEY` in `http/HttpNames.h:64` | `http/EffectiveUrlCache.cc:170` | Enables effective-URL caching. |
| `Http.cache.effective.urls.skip.regex.pattern` | `HTTP_CACHE_EFFECTIVE_URLS_SKIP_REGEX_KEY` in `http/HttpNames.h:65` | `http/EffectiveUrlCache.cc:179` | Skip-pattern for effective-URL caching. |
