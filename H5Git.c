...
hid_t get_Dattr_numb(hid_t pid, int* num_attr_ptr,char *dname,char *error) {

   hid_t dset;
   int num_at;

   if ((dset = H5Dopen(pid,dname))<0) {
      sprintf(error,"h5_das server:  unable to open hdf5 dataset of group %d",pid);
      return -1;
   }
   ...
   if ((num_at = H5Aget_num_attrs(dset))<0) {
      sprintf(error,"h5_das server:  failed to obtain hdf5 attribute in dataset %d",dset);
       return -1;
   }
...
hid_t get_Gattr_numb(hid_t pid, int* num_attr_ptr,char *dname,char *error) {

   hid_t c_group;
   int num_at;

   if ((c_group = H5Gopen(pid,dname))<0) {
      sprintf(error,"h5_das server:  unable to open hdf5 group in group %d",pid);
       return -1;
   }
   ...
   if ((num_at = H5Aget_num_attrs(c_group))<0) {
      sprintf(error,"h5_das server:  failed to obtain hdf5 attribute in group %d",c_group);
      return -1;
   }
...
hid_t get_attr_info (hid_t dset,int index,DSattr_t *attr_inst_ptr,int* ignoreptr,char *error) {

   hid_t ty_id,attrid,space;
   H5T_class_t temp_type;
   hsize_t size[DODS_MAX_RANK], dim_n_size;
   ...
   if ((ty_id = H5Aget_type(attrid)) < 0) {
    ...
    if ((space = H5Aget_space(attrid)) < 0) {
     ...
     if (ndims > DODS_MAX_RANK) {
       strcpy(error,"number of dimensions exceeds hdf5_das server allowed.");
       return -1;
     }

     for (j = 0; j < ndims;j++) {
         if (maxsize[j] == H5S_UNLIMITED) {
           strcpy(error,"unexpected length of dimensions for hdf5_das server");
            return -1;
         }
...
hid_t get_dataset(hid_t pid, char *dname,DS_t* dt_inst_ptr,char *error) {
   ...
   if ((dset = H5Dopen(pid,dname))<0) {
      sprintf(error,"h5_das server:  failed to obtain dataset %s",dname);
      return -1;
   }
   
   if ((datatype = H5Dget_type(dset))<0) {
      sprintf(error,"h5_das server:  failed to obtain datatype from  dataset %s",dname);
      return -1;
   }
      
   if ((dataspace = H5Dget_space(dset))<0) {
      sprintf(error,"h5_das server:  failed to obtain dataspace from  dataset %s",dname);
      return -1;
   }
   ...
   temp_type = H5Tget_class(datatype); 
    ...
    if (temp_type < 0) {
      sprintf(error,"h5_das server:  failed to obtain type class from %d",datatype);
      return -1;
    }
   ...
   /* obtain number of attributes in this dataset. */
    ...
    if ((ndims = H5Sget_simple_extent_dims(dataspace, size, maxsize))<0) {
     ...
     /* check dimension size. */
     if (ndims > DODS_MAX_RANK) {
       strcpy(error,"number of dimensions exceeds hdf5-dods server allowed");
       return -1;
     }
     ...
     for (j = 0; j < ndims;j++) {
         if (maxsize[j] == H5S_UNLIMITED) {
            strcpy(error,"unexpected length of dimensions for hdf5-dods server");
            return -1;
         }
     }
...
int get_data(hid_t dset, void* buf,char *error) {
  ...
  int i;
   ...
   if (H5Dread(dset,datatype,dataspace,dataspace,H5P_DEFAULT,buf)<0){
      sprintf(error,"h5_das server:  failed to read data from  dataset %d",dset);
      printf("error %s\n",error);
      return -1;
     }
   }
 ...
 *-------------------------------------------------------------------------
 */			
int get_strdata(hid_t dset,int strindex,char* allbuf,char* buf,char *error) {
 ...
 *-------------------------------------------------------------------------
 */			
int get_slabdata(hid_t dset,int *offset ,int*step,int*count,int num_dim,hsize_t data_size,void *buf,char *error){
  ...
  if ((datatype = H5Dget_type(dset))<0) {
    sprintf(error,"h5_dods server:  failed to obtain datatype from  dataset %d",dset);
     return 0;
   }
  ...
  if(memtype < 0){
   ...
   if ((dataspace = H5Dget_space(dset))<0) {
      sprintf(error,"h5_dods server:  failed to obtain dataspace from  dataset %d",dset);
      return 0;
   } 
    ...
    if (!dyn_count) {
      sprintf(error,"h5_dods server: out of memory for hyperslab dataset %d",dset);
      return 0;
    }
    ...
    if (H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, dyn_offset, dyn_step,dyn_count, NULL)<0){
      sprintf(error,"h5_dods server: selection error for dataspace",dataspace);
      return 0;
      }
   ...
   if (H5Dread(dset,memtype,memspace,dataspace,H5P_DEFAULT,(void *)buf)<0) {
     sprintf(error,"get_selecteddata: unable to get data for dataset %d",dset);
     return 0;
   }

#if 0
    if (H5Dread(dset,datatype,dataspace,dataspace,H5P_DEFAULT,(void*)tempbuf)<0){
    ...
    if (H5Dread(dset,H5T_NATIVE_INT,H5S_ALL,H5S_ALL,H5P_DEFAULT,(void*)buf)<0){
      fprintf(stdout,"error \n");fflush(stderr);
        return -1;
     }
#endif
...
hid_t get_diminfo(hid_t dataset,int index,int* nelmptr,size_t* dsizeptr,hid_t* dimtypeptr) {
