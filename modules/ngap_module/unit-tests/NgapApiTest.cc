...
public:
    ...
    void test_dmrpp_is_removed_from_data_url()
    {
        ...
        size_t pos1 = url_with_suffix.find(".dmrpp");
        if (pos1 != string::npos) {
            url_with_suffix.erase(pos1, string(".dmrpp").length());
        }
        ...
        size_t pos2 = url_without_suffix.find(".dmrpp");
        if (pos2 != string::npos) {
            url_without_suffix.erase(pos2, string(".dmrpp").length());
        }
        ...
        string expected_middle_removal_url = "https://my-domain.com/path/file.nc";

        size_t pos3 = url_with_middle_dmrpp.find(".dmrpp");
        if (pos3 != string::npos) {
            url_with_middle_dmrpp.erase(pos3, string(".dmrpp").length());
        }
        CPPUNIT_ASSERT_EQUAL_MESSAGE("URL with .dmrpp not at the end should still have it removed.",
                                     expected_middle_removal_url, url_with_middle_dmrpp);
