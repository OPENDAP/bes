...
//
// NgapPathInfoResponseHandler.cc
//
...
NgapPathInfoResponseHandler::NgapPathInfoResponseHandler(const string &name) :
        BESResponseHandler(name), _response(0)
{
}

NgapPathInfoResponseHandler::~NgapPathInfoResponseHandler()
{
}
 ...
 */
void NgapPathInfoResponseHandler::execute(BESDataHandlerInterface &dhi)
{
    BESStopWatch sw;
    if (BESISDEBUG(TIMING_LOG)) sw.start("NgapPathInfoResponseHandler::execute", dhi.data[REQUEST_ID]);

    BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::execute() - BEGIN" << endl );
    ...
    BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::execute() - container: " << container << endl );

    info->begin_response(SHOW_NGAP_PATH_INFO_RESPONSE_STR, dhi);
    ...
    BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::execute() - END" << endl );
 ...
 * @see BESDataHandlerInterface
 */
void NgapPathInfoResponseHandler::transmit(BESTransmitter *transmitter, BESDataHandlerInterface &dhi)
{
    if (_response) {
 ...
 */
void NgapPathInfoResponseHandler::dump(ostream &strm) const
{
    strm << BESIndent::LMarg << "NgapPathInfoResponseHandler::dump - (" << (void *) this << ")" << std::endl;
    BESIndent::Indent();
...
BESResponseHandler *
NgapPathInfoResponseHandler::NgapPathInfoResponseBuilder(const string &name)
{
    return new NgapPathInfoResponseHandler(name);
}

/**
 *
 */
void NgapPathInfoResponseHandler::eval_resource_path(const string &resource_path, const string &catalog_root,
                                                        ...
                                                        long long &lastModifiedTime, bool &canRead, string &remainder)
{
    ...
    BESDEBUG(SPI_DEBUG_KEY,
             "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "CatalogRoot: "<< catalog_root << endl);

    BESDEBUG(SPI_DEBUG_KEY,
             "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "resourceID: "<< resource_path << endl);
    ...
    if (follow_sym_links) {
        BESDEBUG(SPI_DEBUG_KEY,
                 "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "Using 'stat' function (follow_sym_links = true)" << endl);
        ye_old_stat_function = &stat;
    }
    else {
        BESDEBUG(SPI_DEBUG_KEY,
                 "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "Using 'lstat' function (follow_sym_links = false)" << endl);
    ...
    // assumed to be valid.
    if (resource_path == "") {
        BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::"<<__func__ << "() - The resourceID is empty" << endl);
        return;
    }
    ...
    if (dotdot != string::npos) {
        BESDEBUG(SPI_DEBUG_KEY,
                 "NgapPathInfoResponseHandler::"<<__func__ << "() - " << " ERROR: The resourceID '" << resource_path <<"' contains the substring '..' This is Forbidden." << endl);
    ...
    while (!done) {
        ...
        if (slash == string::npos) {
            BESDEBUG(SPI_DEBUG_KEY,
                     "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "Checking final path component: " << rem << endl);
        ...
        BESDEBUG(SPI_DEBUG_KEY,
                 "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "validPath: "<< validPath << endl);
        BESDEBUG(SPI_DEBUG_KEY,
                 "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "checking: "<< checking << endl);
        BESDEBUG(SPI_DEBUG_KEY,
                 "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "fullpath: "<< fullpath << endl);

        BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "rem: "<< rem << endl);

        BESDEBUG(SPI_DEBUG_KEY,
                 "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "remainder: "<< remainder << endl);
        ...
        if (statret != -1) {
            ...
            BESDEBUG(SPI_DEBUG_KEY,
                     "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "error: "<< error << "   errno: " << errno << endl);
            ...
            if (s_loc == string::npos) {
                ...
                while (moreDots) {
                    ...
                    if (d_loc != string::npos) {
                        ...
                        BESDEBUG(SPI_DEBUG_KEY,
                                 "NgapPathInfoResponseHandler::" << __func__ << "() - candidate_path: "<< candidate_path << endl);

                        string full_candidate_path = BESUtil::assemblePath(catalog_root, candidate_path, true);
                        BESDEBUG(SPI_DEBUG_KEY,
                                 "NgapPathInfoResponseHandler::" << __func__ << "() - full_candidate_path: "<< full_candidate_path << endl);
                        ...
                        BESDEBUG(SPI_DEBUG_KEY,
                                 "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "No dots in remainder: "<< remainder << endl);
                        moreDots = false;
                    }
                }
            }
                ...
                BESDEBUG(SPI_DEBUG_KEY,
                         "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "Remainder has slash pollution: "<< remainder << endl);
                done = true;
            }
        }
        ...
        if (S_ISREG(sb.st_mode)) {
            BESDEBUG(SPI_DEBUG_KEY,
                     "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "'"<< fullpath << "' Is regular file." << endl);
            isFile = true;
            isDir = false;
        }
        else if (S_ISDIR(sb.st_mode)) {
            BESDEBUG(SPI_DEBUG_KEY,
                     "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "'"<< fullpath << "' Is directory." << endl);
            isFile = false;
            isDir = true;
        }
        else if (S_ISLNK(sb.st_mode)) {
            BESDEBUG(SPI_DEBUG_KEY,
                     "NgapPathInfoResponseHandler::"<<__func__ << "() - " << "'"<< fullpath << "' Is symbolic Link." << endl);
    ...
    BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::" << __func__ << "() -  fullpath: " << fullpath << endl);
    BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::" << __func__ << "() - validPath: " << validPath << endl);
    BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::" << __func__ << "() - remainder: " << remainder << endl);
    BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::" << __func__ << "() -       rem: " << rem << endl);
    BESDEBUG(SPI_DEBUG_KEY,
             "NgapPathInfoResponseHandler::" << __func__ << "() -    isFile: " << (isFile?"true":"false") << endl);
    BESDEBUG(SPI_DEBUG_KEY,
             "NgapPathInfoResponseHandler::" << __func__ << "() -     isDir: " << (isDir?"true":"false") << endl);
    BESDEBUG(SPI_DEBUG_KEY,
             "NgapPathInfoResponseHandler::" << __func__ << "() -    access: " << (canRead?"true":"false") << endl);
    BESDEBUG(SPI_DEBUG_KEY, "NgapPathInfoResponseHandler::" << __func__ << "() -      size: " << size << endl);
    BESDEBUG(SPI_DEBUG_KEY,
             "NgapPathInfoResponseHandler::" << __func__ << "() -       LMT: " << lastModifiedTime << endl);
