...
/**
 * I think this implementation of the copy constructor is incomplete/inadequate.
 * Review and fix as needed.
 *
 * I think it's complete except that d_dmrpp_rresource can be initialized to nullptr
 * in the init list (since if it's not null the code throws an InternalError). jhrg 3/18/21
 */
NgapContainer::NgapContainer(const NgapContainer &copy_from) :
    ...
    BESDEBUG(MODULE,
             prolog << "BEGIN   object address: " << (void *) this << " Copying from: "
             << (void *) &copy_from << endl);
...
void NgapContainer::_duplicate(NgapContainer &copy_to) {
    ...
    BESDEBUG(MODULE,
             prolog << "BEGIN   object address: " << (void *) this << " Copying to: "
             << (void *) &copy_to << endl);
