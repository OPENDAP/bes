...
m4_define([AT_BESCMD_BINARY_DAP2_RESPONSE_TEST],  [dnl
    ...
    AS_IF([test -n "$baselines" -a x$baselines = xyes],
        ...
        [
            ...
            # While a single baseline file (`foo_bar`) is required, an optional baseline file (`foo_bar.alternate`)
            # is allowed, to support e.g. floating point rounding differences as generated by different processors. 
            # The test will pass if either baseline matches the newly generated output
            AS_IF([[ test -f $baseline && test -f $baseline.alternate ]],
                [
                    # If both baselines exist, try them both before printing any diffs for the user
                    # (so as to only display diffs that demonstrate a failure to match either baseline)
                    AS_IF([[ diff -b -B $baseline.alternate stdout > /dev/null ]],
                        [
                            AS_IF([test -z "$at_verbose"], [echo "diff -b -B \$baseline.alternate stdout"]) 
                        ],
                        [
                            # If this check fails, it means neither baseline is a match 
                            AT_CHECK([diff -b -B $baseline stdout], [0], [ignore])
                        ])
                ],
                [
                    # echo "HERE WE ARE! $baseline"
                    AT_CHECK([diff -b -B $baseline stdout], [0], [ignore])
                ])
        ])
...
m4_define([AT_BESCMD_GDAL_BINARY_FILE_RESPONSE_TEST], [dnl
    ...
    AS_IF([test -n "$baselines" -a x$baselines = xyes],
        ...
        [
            # If both baselines exist, try them both before printing any diffs for the user
            # (so as to only display diffs that demonstrate a failure to match either baseline)
            AS_IF([[ diff -b $baseline.alternate tmp > /dev/null ]],
                [
                    AS_IF([test -z "$at_verbose"], [echo "diff -b \$baseline.alternate tmp"]) 
                ],
                [
                    # If this check fails, it means neither baseline is a match 
                    AT_CHECK([diff -b $baseline tmp], [ignore],)
