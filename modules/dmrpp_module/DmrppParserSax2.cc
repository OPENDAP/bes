    ...
    case parser_start:
        ...
        if (parser->check_attribute("href", attributes, nb_attributes)) {
            ...
            if(EffectiveUrlCache::TheCache()->is_enabled()){
                ...
                http::url *effective_url = EffectiveUrlCache::TheCache()->cache_effective_url(parser->dmrpp_dataset_href);
                BESDEBUG(PARSER, prolog << "The effective URL is: " << effective_url->str() << endl);
            }
        }
        ...
        // Ingest an dmrpp:chunk element and its attributes
            ...
            if (parser->check_attribute("href", attributes, nb_attributes)) {
                ...
                if(EffectiveUrlCache::TheCache()->is_enabled()){
                    ...
                    http::url *effective_url = EffectiveUrlCache::TheCache()->cache_effective_url(data_url);
                    BESDEBUG(PARSER, prolog << "The effective URL is: " << effective_url->str() << endl);
                }
            }
            else {
