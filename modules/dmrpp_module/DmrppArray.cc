...
bool get_next_future(list<std::future<bool>> &futures, atomic_uint &thread_counter, unsigned long timeout, string debug_prefix) {
    ...
    while(!done){
            ...
            if(future_is_valid){
                ...
                if((*futr).wait_for(timeout_ms) != std::future_status::timeout){
                    try {
                        ...
                        BESDEBUG(SUPER_CHUNK_MODULE, debug_prefix << prolog << "Called future::get() on a ready future. success: " <<
                    ...
                    futr++;
                    BESDEBUG(SUPER_CHUNK_MODULE, debug_prefix << prolog << "future::wait_for() timed out. (timeout: " <<
                                                              timeout << " ms) There are currently " << futures.size() << " futures in process. thread_counter: " << thread_counter << endl);
                }
            }
            else {
                BESDEBUG(SUPER_CHUNK_MODULE, debug_prefix << prolog << "The future was not valid. Dumping... " << endl);
                future_finished = true;
            }
        }
        if (futr!=fend && future_finished) {
            futures.erase(futr);
            thread_counter--;
            BESDEBUG(SUPER_CHUNK_MODULE, debug_prefix << prolog << "Erased future from futures list. (Erased future was "
...
/**
 * @brief Reads the SuperChunk, inflates/deshuffles child chunks as required, and copies the values into array.
 *
 * This function reads the data block for the SuperChunk and then performs the inflate/shuffle/insert/etc operations
 * on all of the chunild Chunk objects either in a serial manner or using a concurrent approach.
 *
 * The concurrency behavior of this function is controlled by the values of two global variables:
 * - DmrppRequestHandler::d_use_compute_threads
 * - DmrppRequestHandler::d_max_compute_threads
 *
 * If the value of d_use_compute_threads is false|no then this function will evaluate each Chunk object in turn in a
 * single thread of execution.
 *
 * If the value of d_use_compute_threads is true|yes then this function will utilize std:async() to generate an
 * std::future for each of the Chunks to be processed. The number of furtures (aka threads) is limited to the
 * value of d_max_compute_threads. Since this happens within the possibly multi-threaded retrieval of SuperChunks
 * which means that the number of allowed compute threads, d_max_compute_threads, is shared across
 * all of the SuperChunk threads.
 *
 * @param super_chunk The SuperChunk to process
 * @param array The DmrppAray into which to enshrine the Chunk data values.
 */
void process_super_chunk(const shared_ptr<SuperChunk> &super_chunk, DmrppArray *array)
{
    ...
    if(!DmrppRequestHandler::d_use_compute_threads){
        ...
        process_chunks_concurrent(chunks_to_process, array, constrained_array_shape);
        ...
        while (!done) {
            ...
            if (!super_chunks.empty()){
                ...
                while(thread_started && !super_chunks.empty()) {
                    ...
                    BESDEBUG(SUPER_CHUNK_MODULE, prolog << "Starting thread for " << super_chunk->to_string(false) << endl);
                    ...
                    if (thread_started) {
                        super_chunks.pop();
                        BESDEBUG(SUPER_CHUNK_MODULE, prolog << "STARTED thread for " << super_chunk->to_string(false) << endl);
                    } else {
...
void DmrppArray::read_contiguous()
{
    ...
    if (!DmrppRequestHandler::d_use_transfer_threads || master_chunk_size <= DmrppRequestHandler::d_min_size) {
        ...
        pthread_t threads[DmrppRequestHandler::d_max_transfer_threads];
        memset(&threads[0], 0, sizeof(pthread_t) * DmrppRequestHandler::d_max_transfer_threads);
        ...
        try {
            ...
            for (unsigned int i = 0;
                 i < (unsigned int) DmrppRequestHandler::d_max_transfer_threads && !chunks_to_read.empty(); ++i) {
            ...
            while (num_threads > 0) {
                ...
                if (tid >= DmrppRequestHandler::d_max_transfer_threads) {
                ...
                --num_threads;
                BESDEBUG(SUPER_CHUNK_MODULE, "joined thread: " << (unsigned int) tid << ", there are: " << num_threads << endl);
                ...
                if (status != 0) {
                    ...
                    ++num_threads;
                    BESDEBUG(SUPER_CHUNK_MODULE, "started thread: " << (unsigned int) tid << ", there are: " << num_threads << endl);
                }
            }
            ...
            join_threads(threads, DmrppRequestHandler::d_max_transfer_threads);
...
void DmrppArray::read_chunks_unconstrained()
{
    ...
    BESDEBUG(SUPER_CHUNK_MODULE, prolog << "d_use_transfer_threads: " << (DmrppRequestHandler::d_use_transfer_threads ? "true" : "false") << endl);
    BESDEBUG(SUPER_CHUNK_MODULE, prolog << "d_max_transfer_threads: " << DmrppRequestHandler::d_max_transfer_threads << endl);
    ...
    if (!DmrppRequestHandler::d_use_transfer_threads) {  // Serial transfers
        ...
        while(!super_chunks.empty()) {
            auto super_chunk = super_chunks.front();
            super_chunks.pop();
            BESDEBUG(SUPER_CHUNK_MODULE, prolog << super_chunk->to_string(true) << endl );
            super_chunk->read();
        }
    }
...
{
    BESDEBUG(SUPER_CHUNK_MODULE, prolog << " BEGIN, dim: " << dim << endl);
    ...
    unsigned int last_dim = chunk_shape.size() - 1;
    if (dim == last_dim) {
        BESDEBUG(SUPER_CHUNK_MODULE, prolog << " END, This is the last_dim. chunk: " << chunk->to_string() << endl);
        return chunk;
    }
    else {
        // Not the last dimension, so we continue to proceed down the Recursion Branch.
        for (unsigned int chunk_index = chunk_start; chunk_index <= chunk_end; chunk_index += thisDim.stride) {
            ...
            if (needed){
                BESDEBUG(SUPER_CHUNK_MODULE, prolog << " END, Found chunk: " << needed->to_string() << endl);
                return needed;
            }

        }
    }
    BESDEBUG(SUPER_CHUNK_MODULE, prolog << " END, dim: " << dim << endl);
...
void DmrppArray::read_chunks()
{
    ...
    BESDEBUG(SUPER_CHUNK_MODULE, prolog << "d_use_compute_threads: " << (DmrppRequestHandler::d_use_compute_threads ? "true" : "false") << endl);
    BESDEBUG(SUPER_CHUNK_MODULE, prolog << "d_max_compute_threads: " << DmrppRequestHandler::d_max_compute_threads << endl);
    BESDEBUG(SUPER_CHUNK_MODULE, prolog << "SuperChunks.size(): " << super_chunks.size() << endl);
