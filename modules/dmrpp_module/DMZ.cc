...
void DMZ::load_config_from_keys() {
    // ########################################################################
    // Loads the ELIDE_UNSUPPORTED_KEY (see top of this file for key definition)
 ...
 * @param dmr
 * @param group If the parent is null, add the new var to this group
 ...
 * the Dataset node). Each successive variable can be found
...
DMZ::process_special_structure_data(BaseType *btp, const xml_node &special_structure_data) {
    ...
    if (supported_special_structure_type(btp) == false)
        throw BESInternalError(
            "In dmrpp::process_special_structure_data(): the datatype is not a supported special structure variable",
            __FILE__,
            __LINE__);
    ...
    if (!char_data)
        throw BESInternalError(
            "In dmrpp::process_special_structure_data(): the special_structure_data element doesn't contain special structure data values.",
            __FILE__,__LINE__);
    ...
    if (btp->type() == dods_array_c) {
        ...
        if (ar->is_projected())
            throw BESInternalError(
                "In dmrpp::process_special_structure_data(): currently we don't support subsetting of special_structure_data.",
                __FILE__,__LINE__);
...
void DMZ::process_special_structure_data_internal(DmrppStructure *dmrpp_s, std::vector<u_int8_t> &values,
    ...
    auto vi = dmrpp_s->var_begin();
    auto ve = dmrpp_s->var_end();
    for (; vi != ve; vi++) {
        BaseType *bt = *vi;
        Type t_bt = bt->type();
        if (is_simple_type(t_bt) && t_bt != dods_str_c && t_bt != dods_url_c && t_bt != dods_enum_c && t_bt !=
            dods_opaque_c) {
