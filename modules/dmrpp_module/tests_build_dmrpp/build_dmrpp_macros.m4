...
AT_ARG_OPTION_ARG([baselines],
    [AS_HELP_STRING([--baselines=yes|no], [Build the baseline file for the listed tests])],
    [echo "baselines set to $at_arg_baselines";
     baselines=$at_arg_baselines],[baselines=])

AT_ARG_OPTION_ARG([conf],
    [AS_HELP_STRING([--conf=<file>], [Use <file> for the bes.conf file])],
    [echo "bes_conf set to $at_arg_conf"; bes_conf=$at_arg_conf],
    [bes_conf=bes.conf])

AT_ARG_OPTION_ARG([s3tests],
    [AS_HELP_STRING([--s3tests=yes|no], [Run the tests that read/write to the opendap.tests S3 bucket (creds required)])],
    [echo "s3tests set to $at_arg_s3tests"; s3tests=$at_arg_s3tests],
    [s3tests=no])

AT_ARG_OPTION([Verbose],
    [AS_HELP_STRING([--Verbose], [Very verbose output])],
    [echo "Verbose set to $at_Verbose"; Verbose=$at_Verbose],
    [Verbose=no])
...
# AT_GET_DMRPP_3_20()
#
# Usage: AT_GET_DMRPP_3_20(<test_name>, <input_file>, <baseline>, <params>, <output_file>, [<xfail>])
#
m4_define([AT_GET_DMRPP_3_20],  [dnl
    ...
    gd_verbose=""
    AS_IF([test "$Verbose" != "no"], [gd_verbose="-v -X"])

    TEST_CMD="${GET_DMRPP} -A -b ${BES_DATA_ROOT} ${params} ${gd_verbose} ${input_file}"

    AS_IF([test -z "$at_verbose" -a "$Verbose" == "no"],
          [echo "TEST_CMD: .../get_dmrpp -A -b ${BES_DATA_ROOT} ${params} ${gd_verbose} ${input_file}"])

    AS_IF([test "$Verbose" != "no"], [
        echo "# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --"
...
# 
# Usage: AT_GET_DMRPP_3_20_MAKE_MISSING( <test_name> <input_file> <dmrpp_baseline> <missing_baseline> <params> <output_file> [<xfail>] )
#
