...
unsigned long long DmrppRequestHandler::d_contiguous_concurrent_threshold =
    DMRPP_DEFAULT_CONTIGUOUS_CONCURRENT_THRESHOLD;
 ...
 */
DmrppRequestHandler::DmrppRequestHandler(const string &name) : BESRequestHandler(name) {
    ...
    // This must be done here since direct IO flag for individual variables  should NOT be set for netCDF-4 classic
    // response.
    read_key_value(DMRPP_USE_CLASSIC_IN_FILEOUT_NETCDF, is_netcdf4_classic_response);

#if !HAVE_CURL_MULTI_API
    if (DmrppRequestHandler::d_use_transfer_threads)
        ERROR_LOG("The DMR++ handler is configured to use parallel transfers, but the libcurl Multi API is not "
                  "present, defaulting to serial transfers");
#endif
 ...
 * @param line
 */
void handle_exception(const string &file, int line) try { throw; } catch (const InternalErr &e) {
...
void DmrppRequestHandler::get_dmrpp_from_container_or_cache(BESContainer *container, DMR *dmr) {
    try {
        ...
        if (!container) {
            throw BESInternalFatalError("Null container passed to get_dmrpp_from_container_or_cache", __FILE__,
                                        __LINE__);
        }
 ...
 */
template <class T> void DmrppRequestHandler::get_dds_from_dmr_or_cache(BESContainer *container, T *bdds) {
    ...
    if (dds_cache) {
        const auto cached_dds = dynamic_cast<DDS *>(dds_cache->get(filename));
...
bool DmrppRequestHandler::dap_build_dmr(BESDataHandlerInterface &dhi) {
    ...
    if (!bdmr)
        throw BESInternalError("Cast error, expected a BESDMRResponse object.", __FILE__, __LINE__);
...
bool DmrppRequestHandler::dap_build_dap4data(BESDataHandlerInterface &dhi) {
    ...
    try {
        ...
        if (DmrppRequestHandler::is_netcdf4_enhanced_response && DmrppRequestHandler::is_netcdf4_classic_response)
...
bool DmrppRequestHandler::dap_build_dap2data(BESDataHandlerInterface &dhi) {
    ...
    if (!bdds)
        throw BESInternalError("Cast error, expected a BESDataDDSResponse object.", __FILE__, __LINE__);
...
bool DmrppRequestHandler::dap_build_dds(BESDataHandlerInterface &dhi) {
    ...
    if (!bdds)
        throw BESInternalError("Cast error, expected a BESDDSResponse object.", __FILE__, __LINE__);
...
bool DmrppRequestHandler::dap_build_das(BESDataHandlerInterface &dhi) {
    ...
    if (!bdas)
        throw BESInternalError("Cast error, expected a BESDASResponse object.", __FILE__, __LINE__);
    ...
    try {
        ...
        if (das_cache) {
            const auto cached_das = dynamic_cast<DAS *>(das_cache->get(filename));
...
bool DmrppRequestHandler::dap_build_vers(BESDataHandlerInterface &dhi) {
    ...
    if (!info)
        throw BESInternalFatalError("Expected a BESVersionInfo instance.", __FILE__, __LINE__);
