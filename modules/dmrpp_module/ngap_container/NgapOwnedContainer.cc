...
namespace ngap
{
    ...
    {
        NgapOwnedContainer::d_data_source_location = TheBESKeys::read_string_key(DATA_SOURCE_LOCATION, NgapOwnedContainer::d_data_source_location);
        NgapOwnedContainer::d_use_opendap_bucket = TheBESKeys::read_bool_key(USE_OPENDAP_BUCKET, NgapOwnedContainer::d_use_opendap_bucket);
        NgapOwnedContainer::d_inject_data_url = TheBESKeys::read_bool_key(NGAP_INJECT_DATA_URL_KEY, NgapOwnedContainer::d_inject_data_url);
        ...
        {
            NgapOwnedContainer::d_cmr_cache_size_items = TheBESKeys::read_int_key(CMR_CACHE_THRESHOLD, NgapOwnedContainer::d_cmr_cache_size_items);
            NgapOwnedContainer::d_cmr_cache_purge_items = TheBESKeys::read_int_key(CMR_CACHE_SPACE, NgapOwnedContainer::d_cmr_cache_purge_items);
        ...
        NgapOwnedContainer::d_use_dmrpp_cache = TheBESKeys::read_bool_key(USE_DMRPP_CACHE, NgapOwnedContainer::d_use_dmrpp_cache);
        if (NgapOwnedContainer::d_use_dmrpp_cache)
        {
            NgapOwnedContainer::d_dmrpp_mem_cache_size_items = TheBESKeys::read_int_key(DMRPP_CACHE_THRESHOLD, NgapOwnedContainer::d_dmrpp_mem_cache_size_items);
            NgapOwnedContainer::d_dmrpp_mem_cache_purge_items = TheBESKeys::read_int_key(DMRPP_CACHE_SPACE, NgapOwnedContainer::d_dmrpp_mem_cache_purge_items);
            ...
            NgapOwnedContainer::d_dmrpp_file_cache_size_mb = MEGABYTE * TheBESKeys::read_ulong_key(DMRPP_FILE_CACHE_THRESHOLD,
                                                                                                   NgapOwnedContainer::d_dmrpp_file_cache_size_mb);
            NgapOwnedContainer::d_dmrpp_file_cache_purge_size_mb = MEGABYTE * TheBESKeys::read_ulong_key(DMRPP_FILE_CACHE_SPACE,
                                                                                                         NgapOwnedContainer::d_dmrpp_file_cache_purge_size_mb);
            NgapOwnedContainer::d_dmrpp_file_cache_dir = TheBESKeys::read_string_key(DMRPP_FILE_CACHE_DIR,
                                                                                     NgapOwnedContainer::d_dmrpp_file_cache_dir);
            if (BESUtil::mkdir_p(NgapOwnedContainer::d_dmrpp_file_cache_dir, 0775) != 0)
            {
                ERROR_LOG("DMR++ file cache directory '" + NgapOwnedContainer::d_dmrpp_file_cache_dir + "' error: " + strerror(errno));
            }
    ...
    bool NgapOwnedContainer::get_item_from_dmrpp_cache(string &dmrpp_string) const
    {
        ...
        FileCache::Item item;
        if (NgapOwnedContainer::d_dmrpp_file_cache.get(FileCache::hash_key(get_real_name()), item))
        { // got it
    ...
    {
        if (NgapOwnedContainer::d_inject_data_url)
        { // Hmmm, this is a bit of a hack. jhrg 8/22/24
    ...
    bool NgapOwnedContainer::dmrpp_read_from_opendap_bucket(string &dmrpp_string) const
    {
        ...
        catch (http::HttpError &http_error)
        {
            ...
            case 401:
            case 403:
                ERROR_LOG(prolog + "Looked in the OPeNDAP bucket for the DMRpp for: " + get_real_name() + " but got HTTP Status: " + std::to_string(http_error.http_status()));
