...
#define prolog string("BESXDTransmit::").append(__func__).append("() - ")
    ...
    try {
        ...
        if (ce.function_clauses()) {
            ...
            for (DDS::Vars_iter i = dds->var_begin(); i != dds->var_end(); i++) {
                if ((*i)->send_p()) {
                    RequestServiceTimer::TheTimer()->throw_if_timeout_expired(prolog + "interning " + (*i)->name() , __FILE__, __LINE__);
