...
#define prolog "AggMemberDatasetDimensionCache::" << __func__ << "() - "

namespace agg_util {

AggMemberDatasetDimensionCache *AggMemberDatasetDimensionCache::d_instance = nullptr;
...
unsigned long AggMemberDatasetDimensionCache::getCacheSizeFromConfig()
{
    ...
    if (found) {
        ...
        string msg = "[ERROR] AggMemberDatasetDimensionCache::getCacheSize() - The BES Key " + SIZE_KEY +
...
string AggMemberDatasetDimensionCache::getCacheDirFromConfig()
{
    ...
    if (!found) {
        string msg = "[ERROR] AggMemberDatasetDimensionCache::getSubDirFromConfig() - The BES Key " + CACHE_DIR_KEY +
                     " is not set! It MUST be set to utilize the NcML Dimension Cache. ";
        BESDEBUG("cache", msg << endl);
        throw BESInternalError(msg, __FILE__, __LINE__);
    }
...
string AggMemberDatasetDimensionCache::getDimCachePrefixFromConfig()
{
    ...
    if (found) {
        ...
        string msg = "[ERROR] AggMemberDatasetDimensionCache::getResultPrefix() - The BES Key " + PREFIX_KEY +
                     " is not set! It MUST be set to utilize the NcML Dimension Cache. ";
        BESDEBUG("cache", msg << endl);
        throw BESInternalError(msg, __FILE__, __LINE__);
    }
...
string AggMemberDatasetDimensionCache::getBesDataRootDirFromConfig()
{
    ...
    if (!found) {
        ...
        if (!found) {
            string msg =
                    ((string) "[ERROR] AggMemberDatasetDimensionCache::getStoredResultsDir() - Neither the BES Key ") +
                    BES_CATALOG_ROOT +
 ...
 */
AggMemberDatasetDimensionCache::AggMemberDatasetDimensionCache(const string &data_root_dir, const string &cache_dir,
...
AggMemberDatasetDimensionCache::get_instance(const string &data_root_dir, const string &cache_dir,
                                             const string &result_file_prefix, unsigned long long max_cache_size)
{
    if (d_enabled && d_instance == nullptr && libdap::dir_exists(cache_dir)) {
        d_instance = new AggMemberDatasetDimensionCache(data_root_dir, cache_dir, result_file_prefix,
                                                        max_cache_size);
        d_enabled = d_instance->cache_enabled();
        if (!d_enabled) {
            delete d_instance;
            d_instance = nullptr;
            BESDEBUG("cache", prolog << "Cache is DISABLED" << endl);
        }
        else {
#ifdef HAVE_ATEXIT
            atexit(delete_instance);
#endif
            BESDEBUG("cache", prolog << "Cache is ENABLED" << endl);
        }
    }
...
{
    BESDEBUG("cache", prolog << "Deleting singleton BESStoredDapResultCache instance." << endl);
    delete d_instance;
    d_instance = nullptr;
}
 ...
 */
bool AggMemberDatasetDimensionCache::is_valid(const string &cache_file_name, const string &local_id) const
{
...
void AggMemberDatasetDimensionCache::loadDimensionCache(AggMemberDataset *amd)
{
    ...
    try {
        ...
        if (!is_valid(cache_file_name, local_id)) {
            BESDEBUG("cache",
                     "AggMemberDatasetDimensionCache::loadDimensionCache() - File is not valid. Purging file from cache. filename: "
        ...
        if (get_read_lock(cache_file_name, fd)) {
            BESDEBUG("cache",
                     "AggMemberDatasetDimensionCache::loadDimensionCache() - Dimension cache file exists. Loading dimension cache from file: "
                             << cache_file_name << endl);

            ifstream istrm(cache_file_name.c_str());
            if (!istrm)
                throw libdap::InternalErr(__FILE__, __LINE__,
                                          "Could not open '" + cache_file_name + "' to read cached dimensions.");
            ...
            if (create_and_lock(cache_file_name, fd)) {
                ...
                BESDEBUG("cache", prolog << "Created and locked cache file: " << cache_file_name << endl);
