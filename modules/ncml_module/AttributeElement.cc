...
#include <libdap/DDS.h>    // Needed for a test of the dds version
...
AttributeElement::AttributeElement()
        : NCMLElement(0), _name(""), _type(""), _value(""), _separator(NCMLUtil::WHITESPACE), _orgName(""), _tokens(),
          _pOtherXMLParser(0)
{
...
AttributeElement::processAtomicAttributeAtCurrentScope(NCMLParser &p)
{
    ...
    if (_orgName.empty()) {
        if (p.attributeExistsAtCurrentScope(_name)) {
            ...
            BESDEBUG("ncml", "Didn't find attribute: " << _name << " so adding it with type=" << _type << " and value="
                                                       << _value << endl);
            addNewAttribute(p);
        }
    }
...
{
    NCML_ASSERT_MSG(_type == NCMLParser::STRUCTURE_TYPE,
    ...
    if (!_value.empty()) {
        THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
                               "Found non empty() value attribute for attribute container at scope=" +
    ...
    else // Not renaming, either new one or just a scope specification.
    {
        ...
        if (!pAT) // doesn't already exist
        {
            ...
            BESDEBUG("ncml", "Attribute container was not found, creating new one name=" << _name << " at scope="
                                                                                         << p.getScopeString() << endl);
        }
        else {
            BESDEBUG("ncml",
                     "Found an attribute container name=" << _name << " at scope=" << p.getScopeString() << endl);
        }
    }
...
void
AttributeElement::mutateAttributeAtCurrentScope(NCMLParser &p, const string &name, const string &type,
                                                const string &value)
{
    AttrTable *pTable = p.getCurrentAttrTable();
    VALID_PTR(pTable);
    NCML_ASSERT_MSG(p.attributeExistsAtCurrentScope(name),
                    "Logic error. mutateAttributeAtCurrentScope called when attribute name=" + name +
...
AttributeElement::renameAttributeContainer(NCMLParser &p)
{
    ...
    if (p.isNameAlreadyUsedAtCurrentScope(_name)) {
        THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
                               ...
                               " to new name=" + _name +
                               " failed since an attribute or variable already exists with that name at scope=" +
                               p.getScopeString());
    }
...
AttributeElement::processEndAttribute(NCMLParser &p)
{
    ...
    if (p.isScopeAtomicAttribute()) {
        ...
        NCML_ASSERT_MSG(p.getCurrentAttrTable(),
