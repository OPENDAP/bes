...
m4_define([AT_BESCMD_NETCDF_RESPONSE_TEST_NC4_ENHANCED_GRP],  [dnl
    ...
    AS_IF([test -n "$baselines" -a x$baselines = xyes],
         ...
         # While a single baseline file (`foo_bar.data`) is required, an optional baseline file (`foo_bar.data.alternative`)
         ...
         AS_IF([[ test -f $baseline.data && test -f $baseline.data.alternate ]],
            [
                ...
                AS_IF([[ diff -b -B $baseline.data.alternate tmp > /dev/null ]],
                    [
                        AS_IF([test -z "$at_verbose"], [echo "diff -b -B \$baseline.data.alternate tmp"]) 
                    ],
                    [
