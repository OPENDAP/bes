...
bool FONcMap::compare(Array *tomap) {
    ...
    if (isequal && map->dimension_name(map->dim_begin()) != map->name()) {
        isequal = false;
    }

    // compare the dimension name
    if (isequal && tomap->dimension_name(tomap->dim_begin()) != map->dimension_name(map->dim_begin())) {
        isequal = false;
    }
    ...
    if (isequal) {
        ...
        if (0 != memcmp(map_buf, tomap_buf, map->length() * sizeof(map->var()->type()))) {
            isequal = false;
        }
    }
#if 0
