/*
  ...
  2d variable with latitude and longitude variables that do not have CF
  attributes such as degrees_east and degrees_north.
  
  Compilation instruction:

  %h4cc -I/path/to/hdfeos/include  -L/path/to/hdfeos/lib -o \
  grid_1_2d_xy_no_cf_attr -lhdfeos -lgctp grid_1_2d_xy_no_cf_attr.c

  To generate the test file, run

  %./grid_1_2d_xy_no_cf_attr
...
intn  write_field_1d(int32 gdid);
...
intn write_attr(char *fname)
{
    ...
    /* Set an attribute. */
    status  = SDsetattr(sds_id, "long_name", DFNT_CHAR8, 8,  (VOIDP)"latitude");
    if (status == FAIL) {
    ...
    /* Set an attribute. */
    status  = SDsetattr(sds_id, "long_name", DFNT_CHAR8, 9,  (VOIDP)"longitude");
    if (status == FAIL) {
        fprintf(stderr, "SDsetattr() failed.\n");
        return -1;
    }
...
{
    intn status = 0;
    char filename[]= "grid_1_2d_xy_no_cf_attr.hdf";
