...
m4_define([_AT_BESCMD_TEST], [dnl
    ...
    pass=$3
    repeat=$4
    AS_IF([test -n "$repeat" -a x$repeat = xrepeat -o x$repeat = xcached], [repeat="-r 3"], [repeat=""])
...
[_AT_BESCMD_TEST([$abs_srcdir/$1], [$abs_srcdir/$1.baseline], [$2], [repeat])
])
