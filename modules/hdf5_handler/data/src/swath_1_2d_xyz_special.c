...
herr_t write_dimension(hid_t swid)
{
    HE5_SWdefdim(swid, "Z-Dim", 4);
    HE5_SWdefdim(swid, "N-Dim", 8);
    return SUCCEED;
}
...
herr_t write_swath(hid_t swfid, char* sname)
{
    ...
    write_field_1d(swid, "Pressure-1",  1, 4, "Z-Dim", "hPa");
    write_field_1d(swid, "Latitude-1",  1, 8, "N-Dim", "degrees_north");
    write_field_1d(swid, "Longitude-1", 1, 8, "N-Dim", "degrees_east" );

    /* Define data fields. */
    write_field_2d(swid, "Temperature-1", 0, "Z-Dim,N-Dim", "K");
...
int main()
{
    ...
    /* Create a file. */
    swfid = HE5_SWopen("swath_1_2d_xyz_special_char.h5", H5F_ACC_TRUNC);

    /* Create a Swath. */
    write_swath(swfid, "Swath-1");
