...
main (void)
{
    hid_t       file, grp,gdset,filetype, filetype2, filetype3, filetype4, memtype4, memtype, memtype2, memtype3, space, space2, space3, space4, dset, dset2, dset3, dset4, attr, attr2, attr3, attr4;/* Handles */
    herr_t      status;
    ...
    char        wdata4[4][9] = {"Parting!","is so\0","swe\0er",""};
    /*
    ...
    filetype4 = H5Tcopy (H5T_FORTRAN_S1);
    status   = H5Tset_size (filetype4, 9 - 1);
    if(status < 0) ERROR("Fails to set the total size for H5T_FORTRAN_S1.");
    memtype4  = H5Tcopy (H5T_C_S1);
    status   = H5Tset_size (memtype4, 9);
    if(status < 0) ERROR("Fails to set the total size for H5T_C_S1.");
    ...
    gdset = H5Dcreate2 (grp, DATASET3, filetype4,  space3,  H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    status = H5Dwrite (gdset, memtype4,  H5S_ALL, H5S_ALL, H5P_DEFAULT, wdata4[0]);
    ...
    status = H5Tclose (filetype4);
    status = H5Tclose (memtype);
    status = H5Tclose (memtype2);
    status = H5Tclose (memtype3);
    status = H5Tclose (memtype4);
    status = H5Dclose(gdset);
    status = H5Gclose (grp);
