/*
  A simple test program that generates four Grids with different pixel origin 
  parameter.
  
  To compile this program, run

  %h5cc -I/path/to/hdfeos5/include -L/path/to/hdfeos5/lib -o grid_4_2d_origin \
        -lhe5_hdfeos -lgctp
...
herr_t write_grid(hid_t gdfid, char* gname, int origin);
...
herr_t write_grid(hid_t gdfid, char* gname, int origin)
{
    ...
    /* Set pixel registration. */
    status = HE5_GDdeforigin(gdid, origin);
...
int main()
{
    ...
    /* Create a file. */
    gdfid = HE5_GDopen("grid_4_2d_origin.h5", H5F_ACC_TRUNC);

    /* Write identical grids except names and pixel origin. */
    write_grid(gdfid, "GeoGrid1", HE5_HDFE_GD_UL);
    write_grid(gdfid, "GeoGrid2", HE5_HDFE_GD_UR);
    write_grid(gdfid, "GeoGrid3", HE5_HDFE_GD_LL);
    write_grid(gdfid, "GeoGrid4", HE5_HDFE_GD_LR);
    /* Close the file. */
