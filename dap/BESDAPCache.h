...
#ifndef BESDAPCache_h_
#define BESDAPCache_h_ 1
...
// See BESDAPCache.cc and look at the purge() method.
typedef struct {
    string name;
 ...
 */
class BESDAPCache: public libdap::DapObj {

private:
    static BESDAPCache * d_instance;
    ...
    // Testing
    BESDAPCache(const string &cache_dir, const string &prefix, unsigned long long size);

    // Suppress the assignment operator and default copy ctor, ...
    BESDAPCache();
    BESDAPCache(const BESDAPCache &);
    BESDAPCache &operator=(const BESDAPCache &rhs);
...
public:
    static BESDAPCache *get_instance(const string &cache_dir, const string &prefix, unsigned long long size);
    static BESDAPCache *get_instance();
