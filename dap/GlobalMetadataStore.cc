...
void GlobalMetadataStore::transfer_bytes(int fd, ostream &os)
{
    ...
    if (status != 0)
        ERROR_LOG("Error calling posix_advise() in the GlobalMetadataStore: " << strerror(status) << endl);
#endif
...
void GlobalMetadataStore::insert_xml_base(int fd, ostream &os, const string &xml_base)
{
    ...
    if (status != 0){
        ...
        ERROR_LOG(prolog << "Error calling posix_advise() in the GlobalMetadataStore: " << status_str << endl);
    }
#endif
