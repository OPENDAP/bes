...
public:
    ...
    void store_dap4_result_test()
    {
        ...
        DBG(
            cerr
                << "store_dap4_result_test() - Checking stored result request where the result cache is not configured."
                << endl);
        ...
        DBG(
            cerr << "store_dap4_result_test() - Checking stored result request where async_accpeted is NOT set."
                << endl);
        ...
        DBG(
            cerr
                << "store_dap4_result_test() - Checking stored result request where client indicates that async is accepted (async_accepted IS set)."
                << endl);
        ...
        try {
            ...
            while (i != cached_data->root()->var_end()) {
                ...
                (*i)->print_val(oss, "", false /*print declaration */);
                DBG(
                    cerr << "response_value (" << oss.str().length() << " chars): " << endl << oss.str() << endl
                        << endl);
                ++i;
            }

            DBG(
                cerr << "cache_and_read_a_dap4_response() - baseline ( " << baseline.length() << " chars): " << endl
                    << baseline << endl);
    ...
    void escape_code_test()
    {
        ...
        DBG(cerr << drb3->get_dataset_name() << endl);
        DBG(cerr << drb3->get_ce() << endl);
        ...
        DBG(cerr << drb5->get_dataset_name() << endl);
        DBG(cerr << drb5->get_ce() << endl);
