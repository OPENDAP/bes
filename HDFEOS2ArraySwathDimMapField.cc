...
HDFEOS2ArraySwathDimMapField::read ()
{
    ...
    if (sotype!=DEFAULT_CF_EQU) {
        ...
        if("MODIS_SWATH_Type_L1B" == swathname) {
            ...
            if(fieldname.find(emissive_str)!=string::npos) {
                if(0 == fieldname.compare(fieldname.size()-emissive_str.size(),emissive_str.size(),emissive_str))
                    is_emissive_field = true;
            }

            if(fieldname.find(RefSB_str)!=string::npos) {
                if(0 == fieldname.compare(fieldname.size()-RefSB_str.size(),RefSB_str.size(),RefSB_str))
                    is_refsb_field = true;
            }

            if ((true == is_emissive_field) || (true == is_refsb_field)) 
                throw InternalErr (__FILE__, __LINE__, "Currently don't support MODIS Level 1B swath dim. map for data ");
        }
    }
    ...
    if(true == turn_on_disable_scale_comp_key && false== is_modis1b)
        write_dap_data_disable_scale_comp(sfid,swathid,nelms,offset32,count32,step32);
    else
        write_dap_data_scale_comp(sfid,swathid,nelms,offset32,count32,step32);
...
//#if 0 
int 
HDFEOS2ArraySwathDimMapField::write_dap_data_scale_comp(int32 sfid, int32 swathid, int nelms, vector<int32>& offset32,vector<int32>& count32,vector<int32>& step32) {
    ...
    if (sotype!=DEFAULT_CF_EQU) {
        ...
        if(scale_factor_attr_index!=FAIL)
        {
            ...
            // Appears that the assumption for the datatype of scale_factor is either float or double
            // for this type of MODIS files. So far we haven't found any problems. Maybe this is okay.
            // KY 2013-12-19
            switch(attrtype)
            {
...
int
HDFEOS2ArraySwathDimMapField::write_dap_data_disable_scale_comp(int32 sfid, int32 swathid, int nelms, vector<int32>& offset32,vector<int32>& count32,vector<int32>& step32) {
