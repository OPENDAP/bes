...
void BESRemoteHttpResource::retrieveResource(const string &inject_url) {
    ...
    try {
        ...
        BESDEBUG(MODULE,
                 "BESRemoteHttpResource::retrieveResource() - Caught exception, unlocking cache and re-throw." << endl);
        cache->unlock_cache();
        throw;
    }
...
void BESRemoteHttpResource::setType(const vector<string> *resp_hdrs) {
    ...
    if (!disp.empty()) {
        ...
        BESDEBUG(MODULE,
                 "BESRemoteHttpResource::setType() - Evaluated content-disposition '" << disp << "' matched type: \""
                                                                                      << type << "\"" << endl);
    }
    ...
    if (type.empty() && !ctype.empty()) {
        ...
        BESDEBUG(MODULE,
                 "BESRemoteHttpResource::setType() - Evaluated content-type '" << ctype << "' matched type \"" << type
                                                                               << "\"" << endl);
    }
