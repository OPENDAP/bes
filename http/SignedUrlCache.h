...
private:
    ...
    // TODO: think a little more about splitting functionality to revert control to the caller
    std::shared_ptr<EffectiveUrl> sign_url_with_fetched_credentials(std::shared_ptr<url> source_url);
    static std::shared_ptr<EffectiveUrl> sign_url(const std::string &s3_url, const std::shared_ptr<S3AccessKeyTuple> credentials);
