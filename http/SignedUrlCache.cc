...
// TODO-docstring
bool SignedUrlCache::are_s3credentials_expired(std::shared_ptr<SignedUrlCache::S3AccessKeyTuple> const credentials) {
    // TODO: implement!
    return false;
}
...
shared_ptr<SignedUrlCache::S3AccessKeyTuple> SignedUrlCache::retrieve_cached_s3credentials(string const &url_key) {
    ...
    if (it != d_s3credentials_cache.end()) {
        ...
        if (are_s3credentials_expired(it->second)) {
