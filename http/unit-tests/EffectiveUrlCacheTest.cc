...
/* TESTS BEGIN */
    ...
    void cache_test_00() {
        ...
        try {
            ...
            DBG(cerr << prolog << "EffectiveUrlCache::TheCache()->get_effective_url() returned: " << result_url << endl);
    ...
    void trusted_url_test_01(){
        ...
        try {
            ...
            DBG(cerr << prolog << "source_url: " << untrusted_src_url->str() << " is " << (untrusted_src_url->is_trusted()?"":"NOT ") << "trusted." << endl);
            DBG(cerr << prolog << "result_url: " << result_url->str() << " is " << (result_url->is_trusted()?"":"NOT ") << "trusted." << endl);
            DBG(cerr << prolog << "EffectiveUrlCache::TheCache()->d_effective_urls.size(): " << EffectiveUrlCache::TheCache()->d_effective_urls.size() << endl);
            CPPUNIT_ASSERT( EffectiveUrlCache::TheCache()->d_effective_urls.size() == 1);
            ...
            DBG(cerr << prolog << "source_url: " << trusted_src_url->str() << " is " << (trusted_src_url->is_trusted()?"":"NOT ") << "trusted." << endl);
            DBG(cerr << prolog << "result_url: " << result_url->str() << " is " << (result_url->is_trusted()?"":"NOT ") << "trusted." << endl);
            DBG(cerr << prolog << "EffectiveUrlCache::TheCache()->d_effective_urls.size(): " << EffectiveUrlCache::TheCache()->d_effective_urls.size() << endl);
            CPPUNIT_ASSERT( EffectiveUrlCache::TheCache()->d_effective_urls.size() == 1);
            CPPUNIT_ASSERT( result_url->str() == result_url_str);
