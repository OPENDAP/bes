...
RemoteResource::RemoteResource(std::shared_ptr<http::url> target_url, const std::string &uid, int64_t expiredInterval)
        : d_remoteResourceUrl(std::move(target_url)), d_uid(uid), d_response_headers(new vector<string>()),
          d_http_response_headers(new map<string, string>()), d_expires_interval(expiredInterval) {
