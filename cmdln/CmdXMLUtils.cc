 ...
 */
void
CmdXMLUtils::XMLErrorFunc( void *context, const char *msg, ... )
{
    va_list args ;
    va_start( args, msg ) ;
    char mymsg[1024] ;
 ...
 */
void
CmdXMLUtils::GetProps( xmlNode *node, map< string, string> &props )
{
    if( !node )
    {
	return ;
 ...
 */
void
CmdXMLUtils::GetNodeInfo( xmlNode *node,
			  string &name,
			  string &value,
			  map<string, string> &props )
{
	...
	CmdXMLUtils::GetProps( node, props ) ;
 ...
 */
xmlNode *
CmdXMLUtils::GetFirstChild( xmlNode *node,
			    string &child_name,
			    string &child_value,
			    map<string, string> &child_props )
{
	...
	while( child_node && !done )
	{
		...
		CmdXMLUtils::GetNodeInfo( child_node, child_name,
 ...
 */
xmlNode *
CmdXMLUtils::GetNextChild( xmlNode *child_node,
			  string &next_name,
			  string &next_value,
			  map<string, string> &next_props )
{
	...
	while( child_node && !done )
	{
		...
		CmdXMLUtils::GetNodeInfo( child_node, next_name,
 ...
 */
xmlNode *
CmdXMLUtils::GetChild( xmlNode *node,
		       ...
		       map<string, string> &child_props )
{
	...
	while( child_node && !done )
	{
		...
		if( name == child_name )
		{
		    ...
		    CmdXMLUtils::GetNodeInfo( child_node, name,
					      child_value, child_props) ;
		}
		else
		{
