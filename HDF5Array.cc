...
bool HDF5Array::read(const string & dataset)
{
    ...
    if (return_type(d_ty_id) == "Structure") {
        ...
        DBG(cerr << "=read() Array of Structure length=" << length() <<
            endl);
    ...
    if (return_type(d_ty_id) == "Array") {
        ...
        DBG(cerr << "=read() Array (in Structure) of length=" << length()
            << endl);
            ...
            if (q->is_constructor_type()) {     // Grid, structure or sequence
                if (n == 0) {   // Array at the Bottom level 
                    if (d_type == H5T_INTEGER) {
                        ...
                        if (size == 2) {
                            s1_array2 =
                                H5Tarray_create(H5T_NATIVE_SHORT,
                    ...
                    if (d_type == H5T_FLOAT) {
                        if (size == 4) {
                            s1_array2 =
                                H5Tarray_create(H5T_NATIVE_FLOAT,
                                                d_num_dim, size2, NULL);
                        }
                        if (size == 8) {
                            s1_array2 =
                                H5Tarray_create(H5T_NATIVE_DOUBLE,
                                                d_num_dim, size2, NULL);
                        }
                    }
                    ...
                    if (d_type == H5T_STRING) {
                        ...
                        s1_array2 =
                            H5Tarray_create(str_type, d_num_dim, size2,
                                            NULL);
                    }
                    ...
                    DBG(cerr << n << ": parent_name=" << parent_name <<
                        endl);
            ...
            for (int strindex = 0; strindex < d_num_elm; strindex++) {
                ...
                if (get_strdata(strindex, convbuf, strbuf, size, Msga) < 0) {
                    throw InternalErr(__FILE__, __LINE__,
                                      string
                                      ("dap_h5_handler failed on getting data.\n")
                                      + Msga);
                }
                DBG(cerr << "=read()<get_strdata() strbuf=" << strbuf <<
                    endl);
    ...
    if (return_type(d_ty_id) == "Url") {
        ...
        if (H5Tequal(d_ty_id, H5T_STD_REF_DSETREG) > 0) {
            ...
            hdset_reg_ref_t *rbuf =
                (hdset_reg_ref_t *) malloc(sizeof(hdset_reg_ref_t) *
                                           d_num_elm);

            DBG(cerr << "=read() Got regional reference. " << endl);
            herr_t status =
                H5Dread(d_dset_id, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL,
                        H5P_DEFAULT, rbuf);
            if (status < 0) {
                throw InternalErr(__FILE__, __LINE__,
                                  string
                                  ("dap_h5_handler failed on H5Dread().\n")
                                  + Msga);
            }
            ...
            for (i = 0; i < nelms; i++) {
                ...
                if (rbuf[offset[0] + i * step[0]][0] != '\0') {
                    ...
                    did_r =
                        H5Rdereference(d_dset_id, H5R_DATASET_REGION,
                                       &rbuf[offset[0] + i * step[0]]);
                    H5Iget_name(did_r, (char *) buf2, DODS_NAMELEN);
                    DBG(cerr << "=read() dereferenced name is " << buf2 <<
                        endl);
...
#endif
                    hid_t space_id =
                        H5Rget_region(did_r, H5R_DATASET_REGION,
                                      &rbuf[offset[0] + i * step[0]]);
                    ...
                    case H5S_SEL_POINTS:
                        ...
                        buf =
                            (hsize_t *) malloc(sizeof(hsize_t) * npoints *
                                               ndim);
                        H5Sget_select_elem_pointlist(space_id, 0, npoints,
                                                     buf);
        ...
        if (H5Tequal(d_ty_id, H5T_STD_REF_OBJ) > 0) {
            hobj_ref_t *rbuf =
                (hobj_ref_t *) malloc(sizeof(hobj_ref_t) * d_num_elm);
            DBG(cerr << "=read() Got object reference. " << endl);
            herr_t status =
                H5Dread(d_dset_id, H5T_STD_REF_OBJ, H5S_ALL, H5S_ALL,
                        H5P_DEFAULT, rbuf);
            if (status < 0) {
                throw InternalErr(__FILE__, __LINE__,
                                  string
            ...
            for (i = 0; i < nelms; i++) {
                // Let's assume that URL array is always 1 dimension.
                hid_t did_r =
                    H5Rdereference(d_dset_id, H5R_OBJECT,
                                   &rbuf[offset[0] + i * step[0]]);
                char buf2[DODS_NAMELEN];
                H5Iget_name(did_r, (char *) buf2, DODS_NAMELEN);
                DBG(cerr << "=read() dereferenced name is " << buf2 <<
    ...
    try {
        ...
        if (H5Tis_variable_str(d_ty_id)
            ...
            return read_vlen_string(d_dset_id, d_ty_id, nelms, offset,
                                    step, count);
        }
        ...
        if (nelms == d_num_elm) {
            ...
            if (get_data(d_dset_id, (void *) convbuf, Msga) < 0) {
                throw InternalErr(__FILE__, __LINE__,
                                  string
                                  ("hdf5_dods server failed on getting data.\n")
                                  + Msga);
            }
            ...
            if (check_h5str(d_ty_id)) {
                ...
                for (int strindex = 0; strindex < d_num_elm; strindex++) {
                    ...
                    if (get_strdata
                        (strindex, convbuf, strbuf, elesize, Msga) < 0) {
                        throw InternalErr(__FILE__, __LINE__,
                                          string
                                          ("hdf5_dods server failed on getting data.\n")
                                          + Msga);
            ...
            if ((data_size = nelms * H5Tget_size(d_ty_id)) < 0) {
                throw InternalErr(__FILE__, __LINE__,
                                  string
                                  ("hdf5_dods server failed on getting data size."));
            }
            ...
            if (!get_slabdata(d_dset_id, offset, step, count, d_num_dim,
...
#endif
                if (!get_slabdata
                    (d_dset_id, offset, step, count, d_num_dim,
                     (void *) convbuf, Msga)) {

                    throw InternalErr(__FILE__, __LINE__,
                                      string
                                      ("hdf5_dods server failed on getting hyperslab data.\n")
                                      + Msga);
                }
                // Check URL
                ...
                if (check_h5str(d_ty_id)) {
                    ...
                    for (int strindex = 0; strindex < nelms; strindex++) {
                        char *strbuf = new char[elesize + 1];
                        if (get_strdata
                            (strindex, convbuf, strbuf, elesize,
                             Msga) < 0) {
                            throw InternalErr(__FILE__, __LINE__,
                                              string
                                              ("hdf5_dods server failed on getting data.\n")
...
// public functions to set all parameters needed in read function.
    ...
    int HDF5Array::linearize_multi_dimensions(int *start, int *stride,
                                              int *count, int *picks) {
    ...
    bool HDF5Array::read_vlen_string(hid_t d_dset_id, hid_t d_ty_id,
        ...
        H5Dread(d_dset_id, d_ty_id, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                convbuf2);

        int size_max = 0;
        for (int strindex = 0; strindex < d_num_elm; strindex++) {
            cerr << "convbuf2[" << strindex << "]= " << convbuf2[strindex]
                << endl;
        ...
        for (int strindex = 0; strindex < nelms; strindex++) {
            ...
            if (convbuf2[real_index] != NULL) {
                strncpy(strbuf, convbuf2[real_index],
                        strlen(convbuf2[real_index]));
                strbuf[size_max] = '\0';
                v_str[strindex] = strbuf;
