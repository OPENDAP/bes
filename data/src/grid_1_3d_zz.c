/*
  ...
  %h4cc -I/path/to/hdfeos/include  -L/path/to/hdfeos/lib -o grid_1_3d_zz \
        -lhdfeos -lgctp grid_1_3d_zz.c
...
intn write_field_3d(int32 gdid, char* field_name)
{
    ...
    /* Create a 1d field */
    GDdeffield(gdid, "level", "ZDim", DFNT_FLOAT32, 0);

    status = GDwritefield(gdid, "level", NULL, NULL, NULL, temp1);
    if(status == -1){
        fprintf(stderr, "GDwritefield() failed.\n");
        return -1;
    }
...
{
    intn status = 0;
    char filename[]= "grid_1_3d_zz.hdf";
