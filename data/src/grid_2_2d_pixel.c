/*
  A simple test program that generates two Grids with different pixel 
  registration parameter.
  
  To compile this program, run

  %h5cc -I/path/to/hdfeos5/include -L/path/to/hdfeos5/lib -o grid_2_2d_pixel \
        -lhe5_hdfeos -lgctp
...
herr_t write_grid(hid_t gdfid, char* gname, int pixregcode);
...
herr_t write_grid(hid_t gdfid, char* gname, int pixregcode)
{
    ...
    status = HE5_GDdefpixreg(gdid, pixregcode);
...
int main()
{
    ...
    /* Create a file. */
    gdfid = HE5_GDopen("grid_2_2d_pixel.h5", H5F_ACC_TRUNC);

    /* Write identical grids except names and pixel registration. */
    write_grid(gdfid, "GeoGrid1", HE5_HDFE_CENTER);
    write_grid(gdfid, "GeoGrid2", HE5_HDFE_CORNER);
    /* Close the file. */
