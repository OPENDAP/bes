/*
  ...
  %h4cc -I/path/to/hdfeos/include  -L/path/to/hdfeos/lib -o grid_2_2d_xy \
        -lhdfeos -lgctp grid_2_2d_xy.c
...
intn  write_grid(int32 gdfid, char* gname, intn latlon);
intn  write_field_1d(int32 gdid);
intn  write_field_2d(int32 gdid, char* field_name, int xdim, int ydim);
...
intn write_field_2d(int32 gdid, char* field_name, int xdim, int ydim)
{
    ...
    float32 *temp = (float32*)malloc(xdim*ydim*sizeof(32)); /* longitude-xdim first. */
...
{
    intn status = 0;
    char filename[]= "grid_2_2d_xy.hdf";
