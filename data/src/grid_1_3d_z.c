/*
  ...
  3d variable and 1d level.
  
  Compilation instruction:

  %h5cc -I/path/to/hdfeos5/include  -L/path/to/hdfeos5/lib -o grid_1_3d_z \
        -lhe5_hdfeos -lgctp grid_1_3d_z.c
...
herr_t write_field_1d(hid_t gdid, char* field_name);
...
herr_t write_field_1d(hid_t gdid, char* field_name)
{
    ...
    HE5_GDdeffield(gdid, field_name, "ZDim", NULL, 
                       H5T_NATIVE_FLOAT, 0);
    HE5_GDwritefield(gdid, field_name, start, NULL, count, var);
    return write_attr(gdid, field_name, "units", "hPa");
}
...
int main()
{
    ...
    hid_t  gdfid = FAIL;
    /* Create a file. */
    gdfid = HE5_GDopen("grid_1_3d_z.h5", H5F_ACC_TRUNC);
